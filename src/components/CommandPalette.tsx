...
// ── Task scanner — reads all project tasks from localStorage ──
...
function scanAllTasks(projects: Project[]): TaskResult[] {
    const results: TaskResult[] = [];
    for (const proj of projects) {
        try {
            const raw = localStorage.getItem(TASK_PREFIX + proj.id);
            if (!raw) continue;
                      ...
                      "
                                ...
                                <Command.List className="max-h-[340px] overflow-y-auto overflow-x-hidden p-1.5">
                                        ...
                                        <Command.Group heading="Tasks" className={GROUP_HEADING_CLASS}>
                                            {allTasks.map((task) => (
                                                <Command.Item
                                                    ...
                                                    onSelect={() =>
                                                        handleTaskSelect(task.projectId, task.id)
                                                    }
                                                    className={ITEM_CLASS}
                                                >
                                                    <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-foreground/[0.04] dark:bg-white/[0.04]">
                                                        <CheckSquare
                                                            className={`w-4 h-4 ${task.done ? "text-emerald-500" : "text-muted-foreground/40"}`}
                                                        />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p
                                                            className={`font-semibold text-[13px] truncate ${task.done ? "line-through text-muted-foreground/40" : ""}`}
                                                        >
                                                            {task.title}
                                                        </p>
                                    ...
                                    >
                                        ...
                                        <Command.Item
                                            value="go to analytics dashboard charts"
                                            onSelect={() => handleGoTo("/analytics")}
                                            className={ITEM_CLASS}
                                        >
                                            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-foreground/[0.04] dark:bg-white/[0.04]">
                                        ...
                                        <Command.Item
                                            value="go to team members collaboration"
                                            onSelect={() => handleGoTo("/team")}
                                            className={ITEM_CLASS}
                                        >
                                            <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-foreground/[0.04] dark:bg-white/[0.04]">
